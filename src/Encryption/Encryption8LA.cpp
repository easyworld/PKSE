/**
 * Encryption8LA.cpp - Generation 8 Pokemon Legends: Arceus Encryption/Decryption Implementation
 *
 * Implementation of encryption and decryption utilities.
 * See Encryption8LA.h for detailed documentation.
 */
...
namespace Encryption {
    void shuffleArray8LA(std::span<const std::byte> data, std::span<std::byte> result, uint32_t shuffleValue)
    {
        ...
        const uint32_t index = shuffleValue * BLOCK_COUNT8_LA;
        ...
        const auto end = start + (SIZE_BLOCK8_LA * BLOCK_COUNT8_LA);
        ...
        for (uint32_t block = 0; block < BLOCK_COUNT8_LA; block++)
        {
            // blockPosition[index + block] tells us which block is in position 'block'
            const int srcBlockIndex = blockPosition[index + block];
            const size_t srcOffset = start + (SIZE_BLOCK8_LA * srcBlockIndex);
            const size_t destOffset = start + (SIZE_BLOCK8_LA * block);

            // Copy one block from shuffled position to correct position
            std::memcpy(result.data() + destOffset, data.data() + srcOffset, SIZE_BLOCK8_LA);
        }
    }
    ...
    std::byte* decryptArray8LA(std::span<const std::byte> encryptedData)
    {
        ...
        cryptPokemon(mutableSpan, pv, SIZE_BLOCK8_LA, BLOCK_COUNT8_LA);
        ...
        shuffleArray8LA(mutableSpan, resultSpan, sv);
    ...
    std::byte* encryptArray8LA(std::span<const std::byte> decryptedData, uint32_t personalityValue)
    {
         ...
         *
         * This is the reverse of decryptArray8LA. It takes unencrypted, unshuffled
        ...
        for (uint32_t block = 0; block < BLOCK_COUNT8_LA; block++)
        {
            ...
            const size_t srcOffset = start + (SIZE_BLOCK8_LA * block);
            const size_t destOffset = start + (SIZE_BLOCK8_LA * destBlockIndex);

            // Copy one block from decrypted position to shuffled position
            std::memcpy(shuffledSpan.data() + destOffset, decryptedData.data() + srcOffset, SIZE_BLOCK8_LA);
        }

        // Apply XOR cipher (symmetric operation)
        cryptPokemon(shuffledSpan, personalityValue, SIZE_BLOCK8_LA, BLOCK_COUNT8_LA);

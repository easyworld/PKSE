...
namespace Modals {
    ...
    void drawPokemonDetailsModal(TrainerViewScreen& screen, PKSEFramebuffer& fb) {
        ...
        std::string sub = "等级 " + std::to_string(subLvl) + "     编号 " + dex;
        ...
        if (modernFmt) editRow("能力性格", getNatureName(p->statNature()), 27);
        snprintf(buf, sizeof(buf), "%08X", p->pid());   row("PID", buf);
        snprintf(buf, sizeof(buf), "%u", p->exp());
        if (notGen3) editRow("经验值", buf, 24); else row("经验值", buf);
        editRow("蛋", p->isEgg() ? "是" : "否", 17);
        ...
        if (modernFmt) {
            ...
            else snprintf(buf, sizeof(buf), "（无）");
            editRow("蛋获得日期", buf, 30);
        }
        ...
        if (notGen3) editRow("命运的相遇", p->isFatefulEncounter() ? "是" : "否", 31);
        {
            ...
            const char* pkrs = p->isPokerusInfected() ? "感染中" : p->isPokerusCured() ? "已痊愈" : "无";
            if (p->hasPokerus()) editRow("宝可病毒", pkrs, 22);
            else                 row("宝可病毒", pkrs);
        }
        // Ribbons: count only (there can be dozens); Y / tap opens the full list.
        {
            ...
            if (!rb.empty()) {
                const int ry = iy - scroll;
                row("奖章与证章", std::to_string(rb.size()) + "  (Y)");   // advances iy
        ...
        // Legality summary pinned at the bottom of the left pane (R / tap opens the full issue list).
        {
            ...
            if (legalityRep.ok()) {
                fb.drawText(Lx + 18, ly, "合法性：未发现问题", Color(120, 205, 140), TextStyle::Caption);
            } else {
                const std::string label = "合法性：" + std::to_string(legalityRep.problemCount())
                                        + " 个问题  -  按 R／点击查看";
        ...
        fb.drawText(Cx + 18, colY + 16, "数值", Colors::Text, TextStyle::Heading);
        ...
        fb.drawText(cStat, ty, "能力值",          Colors::TextDim, TextStyle::Caption);
        ty += 24;
        ...
        const S rows[6] = {
            ...
            { "攻击", p->ivATK(), av ? p->avATK() : p->evATK(), dispStat(1, p->statATK()) },
            { "防御", p->ivDEF(), av ? p->avDEF() : p->evDEF(), dispStat(2, p->statDEF()) },
            { "特攻", p->ivSPA(), av ? p->avSPA() : p->evSPA(), dispStat(3, p->statSPA()) },
            { "特防", p->ivSPD(), av ? p->avSPD() : p->evSPD(), dispStat(4, p->statSPD()) },
            { "速度", p->ivSPE(), av ? p->avSPE() : p->evSPE(), dispStat(5, p->statSPE()) },
        };
        ...
        ty += 10;
        {
            ...
            if (s) { fb.drawFilledRoundedRect(Cx + 8, ty - 6, Cw - 16, statRowH - 6, 10, Colors::Selected);
                     fb.drawRoundedRect(Cx + 8, ty - 6, Cw - 16, statRowH - 6, 10, Colors::Accent, 2); }
            fb.drawText(nameX, ty, "异色", s ? Colors::Text : Colors::TextDim);
            // Right-aligned to the panel edge, flush with Nature/Gender/Level below it.
            { const char* sv = isShiny ? "是" : "否"; int sw, sh; fb.measureText(sv, sw, sh);
        ...
        ty += 6;
        {
            ...
            if (s) { fb.drawFilledRoundedRect(Cx + 8, ty - 6, Cw - 16, statRowH - 6, 10, Colors::Selected);
                     fb.drawRoundedRect(Cx + 8, ty - 6, Cw - 16, statRowH - 6, 10, Colors::Accent, 2); }
            fb.drawText(nameX, ty, "性格", s ? Colors::Text : Colors::TextDim);
        ...
        ty += 6;
        {
            ...
            const char* gname = (gv == 0) ? "雄性" : (gv == 1) ? "雌性" : "无性别";
        ...
        ty += 6;
        {
            ...
            if (s) { fb.drawFilledRoundedRect(Cx + 8, ty - 6, Cw - 16, statRowH - 6, 10, Colors::Selected);
                     fb.drawRoundedRect(Cx + 8, ty - 6, Cw - 16, statRowH - 6, 10, Colors::Accent, 2); }
            fb.drawText(nameX, ty, "等级", s ? Colors::Text : Colors::TextDim);
        ...
        // (The old on-panel "随机生成" row was removed -- it's L / the bottom nav bar's "Randomize
        ...
        iy = colY + 22;
        fb.drawText(Rx + 18, iy, "招式", Colors::Text, TextStyle::Heading);
        iy += 46;
        for (int i = 0; i < 4; ++i) {
        ...
        iy += 14;
        fb.drawText(Rx + 18, iy, "携带道具", Colors::TextDim, TextStyle::Caption);
        iy += 30;
        {
            const bool s = (sel == 14);
            if (s) { fb.drawFilledRoundedRect(Rx + 14, iy - 8, Rw - 28, 38, 10, Colors::Selected);
        ...
        // legality/ribbon popup dims it like everything else behind them.
        {
            ...
            const std::string legalSeg = legalityRep.ok() ? "" : "R：合法性  |  ";
        ...
        if (screen.details.legalityOverlay) {
            ...
            fb.drawText(ox + 24, oy + 20, "合法性", Colors::Text, TextStyle::Heading);
            { const char* h = "B／点击：关闭"; int hw, hh; fb.measureText(h, hw, hh, TextStyle::Caption);
              fb.drawText(ox + ow - 24 - hw, oy + 28, h, Colors::TextDim, TextStyle::Caption); }
            int ly = oy + 66;
            if (legalityRep.ok()) {
                fb.drawText(ox + 28, ly, "未发现问题。", Color(120, 205, 140), TextStyle::Body);
            } else {
                for (const auto& is : legalityRep.issues) {
                    ...
                    const char* tag = (is.severity == Legality::Severity::Invalid) ? "[非法]  " : "[警告]  ";
        ...
        if (screen.details.ribbonOverlay) {
            ...
            fb.drawFilledRoundedRect(ox, oy, ow, oh, 16, Colors::Panel);
            fb.drawRoundedRect(ox, oy, ow, oh, 16, Colors::Border, 1);
            fb.drawText(ox + 24, oy + 20, "奖章与证章", Colors::Text, TextStyle::Heading);
            { const char* h = "B／点击：关闭"; int hw, hh; fb.measureText(h, hw, hh, TextStyle::Caption);
              fb.drawText(ox + ow - 24 - hw, oy + 28, h, Colors::TextDim, TextStyle::Caption); }

...
namespace Legality {
    ...
    namespace {
        ...
        const char* statName(int i) {
            static const char* const kNames[6] = { "HP", "攻击", "防御", "速度", "特攻", "特防" };
            return kNames[i];
        }
    ...
    Report analyze(const Pokemon::Pokemon& pk, Enums::GameVersion originGroup) {
        ...
        if (pk.nature() > 24)
            add(r, Severity::Invalid, "性格超出范围（" + std::to_string(pk.nature()) + "）");
        ...
        if (pk.hasAwakeningValues()) {
            ...
            for (int i = 0; i < 6; ++i)
                if (av[i] > 200)
                    add(r, Severity::Invalid, std::string(statName(i)) + "觉醒值超过 200（" + std::to_string(av[i]) + "）");
        }
        ...
        // ---- L1: moves are known ids + no duplicates ----
        {
            ...
            for (int i = 0; i < 4; ++i)
                if (mv[i] != 0 && std::string(Names::getMoveName(mv[i])) == "-")
                    add(r, Severity::Invalid, "槽位 " + std::to_string(i + 1) + " 中存在未知招式");
            for (int i = 0; i < 4; ++i)
                for (int j = i + 1; j < 4; ++j)
                    if (mv[i] != 0 && mv[i] == mv[j]) {
                        add(r, Severity::Invalid, "招式重复：" + std::string(Names::getMoveName(mv[i])));
        ...
        if (Pokemon::getLearnableBits(species, pk.form(), originGroup) != nullptr) {
            for (int i = 0; i < 4; ++i) {
                ...
                if (m != 0 && !Pokemon::isLearnable(species, pk.form(), originGroup, m))
                    add(r, Severity::Warning, "可能无法学会此招式：" + std::string(Names::getMoveName(m)));
            }
        }
        ...
        if (pk.heldItem() != 0) {
            ...
            if (std::string(heldName) == "???")
                add(r, Severity::Warning, "未知的携带道具 ID " + std::to_string(pk.heldItem()));
        }

        // ---- L1: ball / language ranges (skip when unwired == 0) ----
        if (pk.ball() != 0 && pk.ball() > 37)
            add(r, Severity::Invalid, "精灵球 ID 超出范围（" + std::to_string(pk.ball()) + "）");
        if (pk.language() != 0 && (pk.language() > 10 || pk.language() == 6))
            add(r, Severity::Invalid, "无效的语言 ID（" + std::to_string(pk.language()) + "）");
        ...
        if (expLevel < 1 || expLevel > 100)
            add(r, Severity::Invalid, "经验值对应无效等级（" + std::to_string(expLevel) + "）");
        if (pk.level() != 0 && pk.level() != expLevel)
            add(r, Severity::Invalid, "等级 " + std::to_string(pk.level()) +
                                      " 与经验值不符（应为 " + std::to_string(expLevel) + "）");

        // ---- L2: met level <= current level ----
        {
            const uint8_t effLevel = pk.level() != 0 ? pk.level() : expLevel;
            if (pk.metLevel() != 0 && pk.metLevel() > effLevel)
                add(r, Severity::Invalid, "相遇等级 " + std::to_string(pk.metLevel()) +
                                          " 高于当前等级 " + std::to_string(effLevel));
        }
        ...
        // ---- L2: gender vs the species gender ratio (255 genderless / 254 female-only / 0 male-only) ----
        {
            ...
            // form side, as `(form & 1) != gender`.
                ...
                add(r, Severity::Invalid, "仅雌性的宝可梦被设为雄性");
            else if (pi.genderRatio == 0 && g != 0)
                add(r, Severity::Invalid, "仅雄性的宝可梦被设为雌性");
        }
        ...
        // ---- L2: species/form is obtainable in this game (per the personal presence bitmask) ----
        {
            ...
            if (bit != 0 && (pi.presence & bit) == 0)
                add(r, Severity::Invalid, "此游戏中无法获得该宝可梦种类／形态");
        }
        ...
        if (!pk.isEgg()) {
            if (otNameStr.empty())
                add(r, Severity::Warning, "初训家名称为空");
            if (pk.id32() == 0)
                add(r, Severity::Warning, "训练家 ID 为零");
        }

        // ---- L1: name lengths (<=12 UTF-16 chars) ----
        if (pk.nickname().size() > 12)
            add(r, Severity::Warning, "昵称超过 12 个字符");
        if (otNameStr.size() > 12)
            add(r, Severity::Warning, "初训家名称超过 12 个字符");

        // ---- Checksum: a strong regression guard on our own editor (flags corruption / mid-edit) ----
        if (!pk.checksumValid())
            add(r, Severity::Warning, "存储的校验和无效");

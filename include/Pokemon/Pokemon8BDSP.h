/**
 * Pokemon8BDSP.h - Brilliant Diamond/Shining Pearl Pokemon Data Class (PB8)
 *
 * This file defines the Pokemon 8 BS class for Generation 8 Brilliant Diamond /
 * Shining Pearl (BDSP).
 *
 * The PB8 entity is byte-for-byte identical to Sword/Shield's PK8 (same 0x148
 * stored / 0x158 party size, same 4-block + party-stats layout, same shuffle +
 * LCG crypto), so this class is a per-game copy of the Sword/Shield PK8 class with BDSP-only
 * additions. Per the generation-per-class convention each game gets its own
 * class in every layer rather than sharing one behind a mode flag.
 *
 * PB8 Format Details:
...
#ifndef POKEMON_POKEMON8_BDSP_H
#define POKEMON_POKEMON8_BDSP_H
...
namespace Pokemon {
    ...
    /**
     * Pokemon8BDSP - Generation 8 Brilliant Diamond/Shining Pearl Pokemon Class
     *
     * Inherits from Pokemon base class and implements the BDSP (PB8) data format,
     * which shares its byte layout with Sword/Shield's PK8. Handles automatic
     * decryption on construction and provides accessors for all Pokemon properties.
     *
     * Gen 8 Unique Features:
     * - StatNature (mints can change effective nature without changing actual nature)
     * - Home Tracker
     * - BDSP-only DPR-illegal flag (byte 0x52 bit 0, unused in PK8)
     */
    class Pokemon8BDSP final : public Pokemon
    {
    public:
        /**
         * Constructs a PB8 object from encrypted Pokemon data.
         *
         * Process:
         ...
         */
        ~Pokemon8BDSP() override = default;

        // Prevent copying (Pokemon data should not be accidentally copied)
        Pokemon8BDSP(const Pokemon8BDSP&) = delete;
        Pokemon8BDSP& operator=(const Pokemon8BDSP&) = delete;

        // Allow moving for efficient transfers
        Pokemon8BDSP(Pokemon8BDSP&&) noexcept = default;
        Pokemon8BDSP& operator=(Pokemon8BDSP&&) noexcept = default;
        ...
        std::unique_ptr<Pokemon> clone() const override {
            ...
            std::byte* enc = encryptArray8BDSP(std::span<const std::byte>(data.data(), dataSize), ec);
            auto c = std::make_unique<Pokemon8BDSP>(std::span<const std::byte>(enc, dataSize));
            delete[] enc;
            return c;
        }

        /** Storage-format game group (this subclass), NOT the origin Version byte. */
        Enums::GameVersion getGameGroup() const noexcept override { return Enums::GameVersion::BDSP; }
        ...
        /**
         * Gets the BDSP "DPR illegal" flag.
         * Location: 0x52 bit 0 (this byte is unused/padding in Sword/Shield's PK8).
         * BDSP sets this flag on Pokemon it considers illegally obtained; PKHeX
         * exposes it as PB8.IsDprIllegal. Reading it is non-destructive.
         * @return true if the DPR-illegal bit is set
         */
        bool isDprIllegal() const noexcept
        {
            return (static_cast<uint8_t>(data[0x52]) & 0x01) != 0;
        }
        ...
        /** Sets gender (0=Male, 1=Female, 2=Genderless). Location: 0x22 bits 2-3 (PB8). */
        ...
        uint16_t calculateChecksum() const noexcept override {
            ...
            const size_t checksumEnd = std::min(dataSize, SIZE_STORED8_BDSP);
